...
class KD_tree{
    public:
        ...
        nodo *INSERT(nodo *raiz, vector<float> dato, int profundidad);
        void DELETE(vector<float> dato, int profundidad);
...
void KD_tree::DELETE( vector<float> dato, int profundidad ){
    vector<nodo *> tmp;
...
nodo *KD_tree::INSERT(nodo *raiz, vector<float> dato, int profundidad){
    if(raiz == nullptr){
        ...
        if( raiz->datos[i] > dato[i] )
            raiz->nodos[0] = INSERT(raiz->nodos[0], dato, profundidad+1);
        else
            raiz->nodos[1] = INSERT(raiz->nodos[1], dato, profundidad+1);
    }
    return raiz;
}
...
int main()
{
    int DIMENSION = 2; ///1, 2, 3
    int nodos = 7;
    float nodo__[][DIMENSION] {{3, 6, 0}, {17, 15, 2}, {13, 15, 1}, {6, 12, 7}, {9, 1, 5}, {2, 7, 10}, {10, 19, 10}};
    ...
    for(size_t i = 0; i<nodos; ++i)
        arbol.raiz = arbol.INSERT(arbol.raiz, vec_[i], 0);
